...
exports.addUsingSheet = async (req, res) => {
  try {
    ...
    const data1 = rows.rows.map((item) => item.company);
    const user = await Company.find(
      { company_name: { $in: data1 } },
      { company_name: 1, _id: 1 }
    );

    for (let employee of rows.rows) {
      const matchedCompany = user.find(
        (company) => company.company_name === employee.company
      );
      if (matchedCompany) {
        created_by = matchedCompany._id;
      } else {
      }
...
exports.getById = async (req, res) => {
  try {
    ...
    // Extract course IDs from the user object
...
exports.deleteProfile = async (req, res) => {
  try {
    ...
    let users = await Employee.findById(profileId);
