...
import { editEmployeeById, getEmployeeProfileById } from "../../redux/actions/employee/employeeActions.js";
...
const Edit = ({ handleUpdate }) => {
  ...
  const data = coursesReducer?.data || [];
  const filteredCourses = data.map(({ _id, name }) => ({
    _id,
    name,
  }));
  ...
  useEffect(() => {
    if (CompanyId) {
      dispatch(getEmployeeProfileById(CompanyId));
      const getProfileById = async () => {
        const data = await dispatch(getProfile(CompanyId));
  ...
  const optionList = filteredCourses.map((course) => ({
    value: course._id,
    label: course.name,
  }));

  const companyId = authReducer.id;
  useEffect(() => {
    // Fetch company data only if CompanyId is available
    if (companyId) {
      // Dispatch the action to get the profile data based on the id (CompanyId)
      dispatch(getProfile(CompanyId))
        .then((response) => {
          // Once the profile data is received, set it in the state
